...
class BasicBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=padding, dilation=dilation, bias=False)
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.act1 = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(out_channels, out_block_channels, kernel_size=3, padding=padding, dilation=dilation, bias = False)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, downsample=None, attn_layer=None):
        super(Bottleneck, self).__init__()
        out_block_channels = out_channels * self.expansion
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, bias=False) #conv1x1(64,64)
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.act1 = nn.ReLU(inplace=True)
...
class ResNet(BaseModel):
    ...
    def __init__(self, block, layers, in_chans=3, block_args=None):
        ...
        self.layer1 = self._make_layer(block, self.channels[0], layers[0], **block_args)
        self._feature_info.append(FeatureInfo(channel_number=self.channels[0], reduction=2, module_name='layer1'))

        self.layer2 = self._make_layer(block, self.channels[1], layers[1], stride=2, **block_args)
        self._feature_info.append(FeatureInfo(channel_number=self.channels[1], reduction=2, module_name='layer2'))

        self.layer3 = self._make_layer(block, self.channels[2], layers[2], stride=2, **block_args)
        self._feature_info.append(FeatureInfo(channel_number=self.channels[2], reduction=2, module_name='layer3'))

        self.layer4 = self._make_layer(block, self.channels[3], layers[3], stride=2, **block_args)
