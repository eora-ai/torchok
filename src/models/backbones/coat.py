...
class ParallelBlock(nn.Module):
    ...
    def interpolate(self, x, scale_factor, size):
        ...
        img_tokens = F.interpolate(img_tokens, scale_factor=scale_factor, mode='bilinear', align_corners=True)
...
class CoaT(BackboneBase):
    """ CoaT class. """

    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dims=(0, 0, 0, 0), serial_depths=(0, 0, 0, 0),
                 parallel_depth=0, num_heads=0, mlp_ratios=(0, 0, 0, 0), qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 crpe_window=None, **kwargs):
        super().__init__()
        crpe_window = crpe_window or {3: 2, 5: 3, 7: 3}
        self.encoder_channels = embed_dims
    ...
    def forward_features(self, x0, return_interm_layers=False):
        B = x0.shape[0]
        ...
        if return_interm_layers:
            ...
            x1_nocls = self.remove_cls(x1)
            x1_nocls = x1_nocls.reshape(B, H1, W1, -1).permute(0, 3, 1, 2).contiguous()
            feat_out.append(x1_nocls)

            x2_nocls = self.remove_cls(x2)
            x2_nocls = x2_nocls.reshape(B, H2, W2, -1).permute(0, 3, 1, 2).contiguous()
            feat_out.append(x2_nocls)

            x3_nocls = self.remove_cls(x3)
            x3_nocls = x3_nocls.reshape(B, H3, W3, -1).permute(0, 3, 1, 2).contiguous()
            feat_out.append(x3_nocls)
    ...
    def forward(self, x):
        return self.forward_features(x, return_interm_layers=False)

    def forward_neck(self, x):
        return x

    def forward_backbone_features(self, x):
        features = self.forward_features(x, return_interm_layers=True)
        return features, [x] + features
...
def coat_lite_tiny(pretrained=False, **kwargs):
    ...
    if pretrained:
        load_pretrained(model, in_chans=kwargs.get('in_chans', 3))
    return model
