...
from src.models.layers import ConvBnAct, BatchNormAct2d, DropPath, AvgPool2dSame, \
...
def _cfg(url='', **kwargs):
    return {
        ...
        'first_conv': 'stem.conv', 'classifier': 'head.fc', **kwargs
    }
...
class ByobNet(BackboneBase):
    ...
    def __init__(self, cfg: ByobCfg, in_chans=3, output_stride=32,
                 zero_init_last_bn=True, drop_path_rate=0., set_neck=False):
        super().__init__()
    ...
    def init_weights(self, zero_init_last_bn=False):
        ...
        if zero_init_last_bn:
            for m in self.modules():
                ...
                if hasattr(m, 'init_weights') and m != self:
