...
class MMDetectionMAP(Metric):
    ...
    """
    def __init__(self, num_classes: int, iou_thr: float = 0.5, nproc: int = 4, logger: str = 'silent'):
        super().__init__()
    ...
    def compute(self):
        return eval_map(self.preds, self.targets, iou_thr=self.iou_thr, nproc=self.nproc, logger=self.logger)[0]
